...
cubits=f1.concat(f2);else
cubits=f1;cubits.to=generateToArray(f1,"cw");if(options.type==3)
cubits.to=cubits.to.concat(generateToArray(f2,"cw"));rotation=90;break;case "l'":property="x";var f1=getCubits(property,1);var f2=getCubits(property,0);if(options.type==3)
cubits=f1.concat(f2);else
cubits=f1;cubits.to=generateToArray(f1,"cw");if(options.type==3)
...
context.fillStyle=_toHexColor(options.color[options.color.length-1]);context.fillRect(0,0,width,height);context.fillStyle=_toHexColor(color);_drawRoundRect(1,1,width-2,height-2,3);var image=new Image();image.src=canvas.toDataURL();var texture=new THREE.Texture(image);texture.anisotropy=4;texture.needsUpdate=true;colorFaceTexture[color]=texture;return texture;}
function createCubit(point){var materials=[];for(var i=0;i<6;i++){var color=options.color[i];var texture=colorFaceTexture(32,32,color);var m=new THREE.MeshBasicMaterial({map:texture,overdraw:1});materials.push(m);}
var material=new THREE.MeshFaceMaterial(materials);var tessellation=_renderer.type=="Canvas"?3:1;if(!createCubit.box)
